...
class FacialImageProcessing:
    ...
    def show_detection_results(self,draw):
        ...
        df['feat'] = facial_features
        df['age'] = ages
        df['gen'] = genders
        # df['feat'] = df['feat'].apply(lambda x: np.mean(x, axis=0))
        df['age'] = df['age'].apply(lambda x: np.median(x))
        df['gen'] = df['gen'].apply(lambda x: np.median(x))
        # print(df.head())
        for i in range(1024):
            df[i] = [x[i] for x in df['feat']]
        df.drop('feat', axis=1, inplace=True)

        d = np.expand_dims(df, axis=2)
        coh, emotion = model.predict(d)
        emotion = np.argmax(emotion)
        coh = int(round(coh[0][0]))
        ...
        for i,b in enumerate(bounding_boxes):
            ...
            cv2.putText(draw, coh, (x2 - 30, y1 + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0))
            cv2.putText(draw, emotion, (x2 - 50, y2 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0))
...
def process_all_images(args, save_processed=False):
    ...
    draw = cv2.imread(args)
    height, width, channels = draw.shape
    if width>640 or height>480:
        draw=cv2.resize(draw, (min(width,640),min(height,480)))
    #draw=cv2.resize(draw, (192,192))
    draw,_=imgProcessing.show_detection_results(draw)
    cv2.imshow(args, draw)
